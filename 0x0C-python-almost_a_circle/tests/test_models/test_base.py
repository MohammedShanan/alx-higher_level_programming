...
# run with python -m unittest tests/test_models/test_base.py
"""
...
class TestBase(unittest.TestCase):
    ...
    def test_to_json_string(self):
        ...
        d1 = {'id': 1, 'size': 3, 'x': 4, 'y': 5}
        strd0 = Base.to_json_string([d0])
        strd1 = Base.to_json_string([d1])
        self.assertTrue(type(d0) == dict)
        self.assertTrue(type(strd0) == str)
        self.assertEqual(
            strd0, '[{"id": 1, "width": 2, "height": 3, "x": 4, "y": 5}]')
        self.assertEqual(strd1, '[{"id": 1, "size": 3, "x": 4, "y": 5}]')
